...
export default function Reflex() {
  ...
  function sendProps(id, name, details, price) {
    Router.push({
      pathname: "/pages/editcustomprod",
      query: {
        id,
        name,
        details,
  ...
  async function delete_doc(id) {
    await deleteDoc(doc(data, "customprod", id));
    const collref = collection(data, "customprod");
    ...
    const coll2 = collection(data, "log");
          ...
          await setDoc(doc(data, "log", ids), {
            change: "Deleted Custom Product : " + "ID:" + id.toString(),
            user: use,
            timestamp: Date.now()
          });
    getRetail();
                      ...
                      {reflex.map((reflex) => {
                        return (
                          ...
                          >
                            ...
                            <td className="text-sm text-gray-900 font-light max-w-3xl px-6 py-4 ">
                              {reflex.details}
                            </td>
                            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                              <Prodimg id={reflex.id - 2} />
                            </td>
                            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                              {reflex.price}
                            </td>
                            ...
                            <td className="text-sm grid grid-cols-2  gap-5 text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                              ...
                              <a
                                className="w-6 hover:text-red-800 cursor-pointer h-6"
                                onClick={() => {
