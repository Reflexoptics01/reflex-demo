...
export default function AddYoutube() {
  ...
  async function addretailer() {
    ...
    var id;
    const querySnapshot = await getDocs(collection(data, "pricelist"));
    const coll = collection(data, "pricelist");
    ...
    await setDoc(doc(data, "pricelist", id), {
      id: id,
      name: name,
      details: details,
      link: link,
    ...
    await setDoc(doc(data, "log", ids), {
      change: "Added Pricelist : " + "title:" + name,
       user: use,
            timestamp:Date.now()
    });
