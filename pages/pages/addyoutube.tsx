...
export default function AddYoutube() {
  ...
  async function addretailer() {
    var id;
    const querySnapshot = await getDocs(collection(data, "youtube"));
    const coll = collection(data, "youtube");
    ...
    await uploadBytes(imageref, image).then( () => {
      getDownloadURL(imageref).then(async (ur) => {
        setUrl(ur);
        await setDoc(doc(data, "youtube", id), {
          id: id,
          name: name,
          details: details,
          url: ur,
        ...
        await setDoc(doc(data, "log", ids), {
          change: "Added Youtube Link : " + "title:" + name,
           user: use,
            timestamp:Date.now()
        });
        Router.push({
          pathname: "/pages/youtube",
        });
      });
    });
  }
