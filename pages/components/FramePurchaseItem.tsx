...
export interface FrameItem extends BasePurchaseItem {
  modelNumber: string;
  size: string;
  frameType: string;
  shape: string;
...
export function FramePurchaseItem({
  item,
  index,
  showDetails,
  onUpdateItem,
  onToggleDetails,
}: Omit<PurchaseItemProps<FrameItem>, 'renderAdditionalFields'>) {
  const renderFrameFields = (frame: FrameItem, onUpdate: (id: string, field: keyof FrameItem, value: string) => void) => {
    return (
      <>
